...
def run_network(inputs, viewdirs, obj, fn, embed_fn, embeddirs_fn, embedobj_fn, netchunk=1024*64):
    """Prepares inputs and applies network 'fn'."""

    inputs_flat = tf.reshape(inputs, [-1, inputs.shape[-1]])

    embedded = embed_fn(inputs_flat)
    embedded_obj = embedobj_fn(obj[1])
    embedded_obj = tf.broadcast_to(embedded_obj, embedded.shape[:-1] + embedded_obj.shape[-1:])
...
def render_rays(ray_batch, obj, network_fn, network_query_fn, N_samples,
    ...
    Args:
      ...
      network_fn: function. Model for predicting RGB and density at each point
        in space.
	  obj: tuple. (int, array of shape [diversity]). Contains index of obj and 1/diversity-hot vector of currently training obj.
    ...
    def raw2outputs(raw, z_vals, rays_d):
        ...
        def raw2alpha(raw, dists, act_fn=tf.nn.relu): return 1.0 - tf.exp(-(act_fn(raw)+1e-10) * dists)
...
def render(H, W, focal, obj,
    ...
    near, far = near[obj[0]] * tf.ones_like(rays_d[..., :1]), far[obj[0]] * tf.ones_like(rays_d[..., :1])
...
def create_nerf(args):
    ...
    embedobj_fn, input_ch_obj = get_embedder(args.multires_obj, args.i_embed, dims = args.diversity)
    ...
    model = init_nerf_model(
        ...
        input_ch_views=input_ch_views, input_ch_obj=input_ch_obj, use_viewdirs=args.use_viewdirs)
...
def train():
    ...
    ARG_DICT = {"expname": "dvox_paper_cube_objembed_short", 
    ...
    if args.render_only:
        ...
        for i in range(args.diversity):
            ...
            obj = (i, tf.one_hot(i, args.diversity, on_value = 1/args.diversity))
    ...
    for i in range(start, N_iters):
        ...
        for d in range(args.diversity):
            ...
            with tf.GradientTape() as tape:
                ...
                obj = (d, tf.one_hot(d, args.diversity, on_value = 1/args.diversity))
        ...
        if i % args.i_testset == 0 and i > 0:
            for d in range(args.diversity):
                ...
                os.makedirs(testsavedir, exist_ok=True)
                print('test poses shape', poses[d][i_test].shape)
                render_path(poses[i_test], hwf[d], obj, args.chunk, render_kwargs_test, gt_imgs=images[d][i_test[d]], savedir=testsavedir)
