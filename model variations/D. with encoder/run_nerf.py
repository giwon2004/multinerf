...
def run_network(inputs, viewdirs, obj, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    obj = tf.broadcast_to(obj, embedded.shape[:-1] + obj.shape[-1:])
...
def render_rays(ray_batch, obj, network_fn, network_query_fn, N_samples,
    ...
    Args:
      ...
      network_fn: function. Model for predicting RGB and density at each point
        in space.
	  obj: array of shape [1, output_ch_obj]. The encoded vector of currently training obj.
...
def create_nerf(args):
    ...
    model = init_nerf_model(
        ...
        input_ch_views=input_ch_views, input_ch_obj=args.output_ch_obj,
        use_viewdirs=args.use_viewdirs)
    grad_vars = model.trainable_variables
    models = {'model': model}

    encoder = init_encoder(args.image_shape, output_ch=args.output_ch_obj)
    grad_vars += encoder.trainable_variables
...
def train():
    ...
    ARG_DICT = {"expname": "dvox_paper_cubegreek_encode_test2", 
    ...
    base_image = [None] * args.diversity
    ...
    for i in range(args.diversity):
        ...
        if dataset_type == 'llff':
            ...
            base_image[i] = tf.image.resize(images[i][:1], args.image_shape)
            hwf[i] = poses[i][0, :3, -1]
            poses[i] = poses[i][:, :3, :4]
    ...
    if args.render_only:
        print('RENDER ONLY')
        for i in range(args.diversity):
            bds_dict = {'near': near[i], 'far': far[i]}
            ...
            obj = models['encoder'](base_image[i])
    ...
    for i in range(start, N_iters):
        ...
        for d in range(args.diversity):
            ...
            # Bring near and far value of d-th dataset
            bds_dict = {'near': near[d], 'far': far[d]}
            ...
            if use_batching:
                ...
                # Choose base image
                if args.random_base:
                    img_b = np.random.choice(i_train[d])
                    base = tf.image.resize(images[d][img_b:img_b+1], args.image_shape)
                else:
                    base = base_image[d]
        ...
        if i % args.i_video == 0 and i > 0:
            for d in range(args.diversity):
                bds_dict = {'near': near[d], 'far': far[d]}
                render_kwargs_test.update(bds_dict)
                obj = models['encoder'](base_image[d])
