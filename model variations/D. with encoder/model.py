...
def init_nerf_model(D=8, W=256, input_ch=3, input_ch_views=3, input_ch_obj=10, output_ch=4, skips=[4], use_viewdirs=False):
    ...
    inputs = tf.keras.Input(shape=(input_ch + input_ch_obj + input_ch_views))
...
def init_encoder(image_shape = [256, 256], output_ch = 10, act = "relu"):
    inputs = tf.keras.Input(shape=image_shape + [3])
    outputs = inputs
    while outputs.shape[1:3] != [1, 1]:
        outputs = tf.keras.layers.Conv2D(filters=output_ch, kernel_size=3, activation=act, padding="same", strides=(2,2), input_shape=image_shape)(outputs)
