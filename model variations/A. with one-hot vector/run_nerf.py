...
def run_network(inputs, viewdirs, obj, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    obj = tf.broadcast_to(obj[1], embedded.shape[:-1] + obj[1].shape[-1:])
...
def render_rays(ray_batch, obj, network_fn, network_query_fn, N_samples,
    ...
    Args:
      ...
      network_fn: function. Model for predicting RGB and density at each point
        in space.
	  obj: tuple. (index of obj, one-hot vector of shape [diversity]).
...
def create_nerf(args):
    ...
    model = init_nerf_model(
        ...
        input_ch_views=input_ch_views, input_ch_obj=args.diversity,use_viewdirs=args.use_viewdirs)
...
def train():
    ...
    if args.render_only:
        ...
        for i in range(args.diversity):
            ...
            print('test poses shape', render_poses[i].shape)

            obj = (i, tf.one_hot(i, args.diversity))
    ...
    for i in range(start, N_iters):
        ...
        for d in range(args.diversity):
            ...
            with tf.GradientTape() as tape:
                ...
                obj = (d, tf.one_hot(d, args.diversity))
